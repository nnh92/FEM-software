...
class Calcrate:
    ...
    ##     ３次元　小軸力｛ｆ’｝＝［Ｋ’］＊［Ｔ］＊｛ｕ｝           ##
    ##                 　 ** FBUZAi.for **                      ##
    ## Chuyển ma trận local sang global
    ##############################################################
    def fbuzai(self, jEL):
    ...
    def SkYマトリックス(self, NEQ):
        ...
        # --- Khởi tạo iD: 0 = free, >0 = DOF tham gia ---
        self.iD = np.ones((6, self.節点数), dtype=int)
        idx_nodes = self.拘束条件節点 - 1

        # Gán 0 cho các DOF bị khóa theo nxfx, nyfx...
        self.iD[0, idx_nodes[self.nxfx != 0]] = 0
        self.iD[1, idx_nodes[self.nyfx != 0]] = 0
        self.iD[2, idx_nodes[self.nzfx != 0]] = 0
        self.iD[3, idx_nodes[self.mxfx != 0]] = 0
        self.iD[4, idx_nodes[self.myfx != 0]] = 0
        self.iD[5, idx_nodes[self.mzfx != 0]] = 0

        # --- Đánh số thứ tự cho các DOF tham gia (iD > 0) ---
        flat = self.iD.flatten()
        idx_free = np.where(flat == 1)[0]                   # vị trí phần tử bằng 1
        flat[idx_free] = np.arange(1, len(idx_free) + 1)    # Gán lại các phần tử này
        self.iD = flat.reshape(self.iD.shape)               # reshape lại về kích thước cũ

        ND = 12         # bậc tự do mỗi phần tử
        ...
        for jEL in range( self.要素数):
            j1, j2 = self.要素節点[jEL][0]-1, self.要素節点[jEL][1]-1
            LM[:6] = self.iD[:, j1]
            LM[6:] = self.iD[:, j2]
        ...
        # --- Lấp dữ liệu vào ma trận Skyline ---
        for jEL in range(self.要素数):
                    ...
                    for j in range(ND):
                        ...
                        if jj > 0:
                            ...
                            if ij >= 0:
                                ...
                                kSS = kS if j < i else j + NDi
                                self.AjCB[NEC] += self.SE[kSS]
